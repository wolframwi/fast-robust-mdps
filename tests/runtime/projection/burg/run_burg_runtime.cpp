...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--instance" && i + 1 < argc) {
      ...
      std::cout
          << "Usage: run_burg_runtime [--instance PATH] [--amb PATH|NAME] [--solver fast|mosek|all]"
             " [--a10-only]\n";
      return 0;
    } else {
  ...
  std::filesystem::path runtime_dir = std::filesystem::path("burg") / "runtime";
  std::filesystem::path error_dir = std::filesystem::path("burg") / "errors";
    ...
    for (const auto& amb_path : amb_files) {
      ...
      for (int s = 0; s < S; ++s) {
        for (int a = 0; a < A; ++a) {
          ...
          if (run_mosek) {
            try {
              mosek_burg::ProjectionInput in_mosek{pbar, V, beta};
              auto t0 = std::chrono::steady_clock::now();
              auto result = mosek_burg::solve_projection_problem(in_mosek);
              auto t1 = std::chrono::steady_clock::now();
              std::chrono::duration<double, std::micro> dt = t1 - t0;
          ...
          if (ok_fast && ok_mosek) {
            ...
            double obj_fast = projection_utils::burg_divergence(p_fast, pbar);
            double obj_mosek = projection_utils::burg_divergence(p_mosek, pbar);
