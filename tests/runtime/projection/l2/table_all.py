...
def main():
    ...
    print(r"\centering")
    print(r"\caption{L2 projection runtimes (all ambiguity levels).}")
    ...
    print(r" & Fast & CPLEX & Gurobi & Mosek & Fast & CPLEX & Gurobi & Mosek & Fast & CPLEX & Gurobi & Mosek \\")
    print(r"\hline")
    ...
    for (s, a) in sorted(synthetic.keys()):
        ...
        for amb in amb_levels:
            ...
            if entries:
                ...
                cplex_vals = [float(e["cplex_median_us"]) for e in entries]
                ...
                cplex = sum(cplex_vals) / len(cplex_vals)
                gurobi = sum(gurobi_vals) / len(gurobi_vals)
                mosek = sum(mosek_vals) / len(mosek_vals)
                cells.append(f"{fmt(fast)} & {fmt(cplex)} & {fmt(gurobi)} & {fmt(mosek)}")
            else:
                cells.append("-- & -- & -- & --")
    ...
    for name in benchmark_names:
        ...
        for amb in amb_levels:
            ...
            if entries:
                if "gurobi_median_us" not in entries[0]:
                    cells.append("-- & -- & -- & --")
                    continue
                fast = float(entries[0]["fast_median_us"])
                cplex = float(entries[0]["cplex_median_us"])
                gurobi = float(entries[0]["gurobi_median_us"])
