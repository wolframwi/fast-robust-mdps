...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--instance" && i + 1 < argc) {
      ...
      std::cout
          << "Usage: run_l2_runtime [--instance PATH] [--amb PATH|NAME] "
    ...
    for (const auto& amb_path : amb_files) {
      ...
      for (int s = 0; s < S; ++s) {
        for (int a = 0; a < A; ++a) {
          ...
          if (run_fast) {
            try {
              fast_l2::ProjectionInput in_fast{pbar, V, weights, beta};
              auto t0 = std::chrono::steady_clock::now();
              auto result = fast_l2::solve_projection_problem(in_fast);
          ...
          if (run_mosek) {
            try {
              mosek_l2::ProjectionInput in_mosek{pbar, V, weights, beta};
              auto t0 = std::chrono::steady_clock::now();
              auto result = mosek_l2::solve_projection_problem(in_mosek);
              auto t1 = std::chrono::steady_clock::now();
              std::chrono::duration<double, std::micro> dt = t1 - t0;
              mosek_times.push_back(dt.count());
              p_mosek = result.p;
              obj_mosek = projection_utils::l2_distance(p_mosek, pbar, weights);
              ok_mosek = true;
            } catch (const std::exception& exc) {
          ...
          if (run_gurobi) {
            try {
              gurobi_l2::ProjectionInput in_gurobi{pbar, V, weights, beta};
              auto t0 = std::chrono::steady_clock::now();
              auto result = gurobi_l2::solve_projection_problem(in_gurobi, *gurobi_env);
              auto t1 = std::chrono::steady_clock::now();
              std::chrono::duration<double, std::micro> dt = t1 - t0;
              gurobi_times.push_back(dt.count());
              p_gurobi = result.p;
              obj_gurobi = projection_utils::l2_distance(p_gurobi, pbar, weights);
              ok_gurobi = true;
            } catch (const std::exception& exc) {
          ...
          if (run_cplex) {
            try {
              cplex_l2::ProjectionInput in_cplex{pbar, V, weights, beta};
              auto t0 = std::chrono::steady_clock::now();
              auto result = cplex_l2::solve_projection_problem(in_cplex, *cplex_env);
              auto t1 = std::chrono::steady_clock::now();
              std::chrono::duration<double, std::micro> dt = t1 - t0;
              cplex_times.push_back(dt.count());
              p_cplex = result.p;
              obj_cplex = projection_utils::l2_distance(p_cplex, pbar, weights);
              ok_cplex = true;
            } catch (const std::exception& exc) {
