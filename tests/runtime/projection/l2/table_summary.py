...
def main():
    ...
    print(r"\centering")
    print(r"\caption{L2 projection runtimes (moderate ambiguity).}")
    print(r"\begin{tabular}{l|cccc}")
    print(r"Instance & Fast (us) & CPLEX (us) & Gurobi (us) & Mosek (us) \\")
    print(r"\hline")
    ...
    for (s, a) in sorted(synthetic.keys()):
        ...
        label = f"S={s},A={a}"
        print(f"{label} & {fmt(fast)} & {fmt(cplex)} & {fmt(gurobi)} & {fmt(mosek)} \\\\")

    for instance, data in sorted(benchmarks, key=lambda x: x[0]):
        label = os.path.basename(instance)
        if "gurobi_median_us" not in data:
            continue
        fast = float(data["fast_median_us"])
        cplex = float(data["cplex_median_us"])
        gurobi = float(data["gurobi_median_us"])
