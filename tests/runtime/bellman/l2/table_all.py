...
def main():
    ...
    print(r"\centering")
    print(r"\caption{L2 Bellman runtimes (all ambiguity levels).}")
    print(r"\begin{tabular}{l|ccc|ccc|ccc}")
    print(r"Instance & \multicolumn{3}{c|}{Mild} & \multicolumn{3}{c|}{Moderate} & \multicolumn{3}{c}{Severe} \\")
    print(r" & Fast & CPLEX & Mosek & Fast & CPLEX & Mosek & Fast & CPLEX & Mosek \\")
    print(r"\hline")
    ...
    for (s, a) in sorted(synthetic.keys()):
        ...
        for amb in amb_levels:
            ...
            if entries:
                ...
                cells.append(f"{fmt(fast)} & {fmt(cplex)} & {fmt(mosek)}")
            else:
                cells.append("-- & -- & --")
