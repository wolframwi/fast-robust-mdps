...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--instance" && i + 1 < argc) {
      ...
      std::cout
          << "Usage: run_l1_bellman_runtime [--instance PATH] [--amb PATH|NAME] "
  ...
  for (const auto& dir : instance_dirs) {
    ...
    for (const auto& amb_path : amb_files) {
      ...
      for (int sample = 0; sample < 100; ++sample) {
        ...
        bellman_in.kappa = data.amb.kappa_l1;
        ...
        if (run_gurobi) {
          try {
            auto t0 = std::chrono::steady_clock::now();
            gurobi_l1_bellman::BellmanInput bench_in;
            bench_in.n_states = S;
            bench_in.n_actions = A;
            bench_in.discount = data.mdp.discount;
            bench_in.kappa = data.amb.kappa_l1;
            ...
            bench_in.transitions = transitions_sa;
            bench_in.sigma = sigma_sa;
            val_gurobi = gurobi_l1_bellman::solve_bellman_state(bench_in, *gurobi_env);
            auto t1 = std::chrono::steady_clock::now();
            std::chrono::duration<double, std::micro> dt = t1 - t0;
        ...
        if (run_mosek) {
          try {
            auto t0 = std::chrono::steady_clock::now();
            mosek_l1_bellman::BellmanInput bench_in;
            bench_in.n_states = S;
            bench_in.n_actions = A;
            bench_in.discount = data.mdp.discount;
            bench_in.kappa = data.amb.kappa_l1;
            ...
            bench_in.transitions = transitions_sa;
            bench_in.sigma = sigma_sa;
            val_mosek = mosek_l1_bellman::solve_bellman_state(bench_in);
            auto t1 = std::chrono::steady_clock::now();
            std::chrono::duration<double, std::micro> dt = t1 - t0;
