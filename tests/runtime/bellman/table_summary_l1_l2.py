...
def main():
    ...
    print(r"\centering")
    print(r"\caption{L1 vs.\ L2 Bellman runtimes (benchmarks, ms).}")
    ...
    for instance in instances:
        ...
        l1_homotopy = median_value(l1_homotopy_entries, "homotopy_median_us")
        ...
        print(
            f"{label} & {fmt_us_to_ms(l1_fast)} & {fmt_us_to_ms(l1_cplex)} & "
            f"{fmt_us_to_ms(l1_gurobi)} & {fmt_us_to_ms(l1_mosek)} & "
            f"{fmt_us_to_ms(l1_homotopy)} & {fmt_us_to_ms(l2_fast)} & "
            f"{fmt_us_to_ms(l2_cplex)} & {fmt_us_to_ms(l2_gurobi)} & "
            f"{fmt_us_to_ms(l2_mosek)} \\\\"
        )
