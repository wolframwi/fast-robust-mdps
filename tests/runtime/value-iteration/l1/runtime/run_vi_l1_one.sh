...
usage() {
  cat <<'EOF'
Usage: run_vi_l1_one.sh <instance_dir> <amb_file> <method>

Runs one L1 value-iteration method and merges the results into runtime/errors.
...
binary="${runtime_dir}/vi_l1_single"
source="${base_dir}/run_vi_l1_single.cpp"
...
if [ ! -x "${binary}" ] || [ "${source}" -nt "${binary}" ]; then
  ...
  sources=(
    ...
    "${root}/algorithms/fast/l1/projection/project.cpp"
