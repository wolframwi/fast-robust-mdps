...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--instance" && i + 1 < argc) {
      ...
      std::cout
          << "Usage: run_l1_vi_runtime [--instance PATH] [--amb PATH|NAME] "
      ...
      if (run_cplex) {
        try {
          ...
          auto cplex_op = [&](int s, const std::vector<double>& v) {
            ...
            in.transitions = transitions_by_state[s];
            in.sigma = sigma_by_state[s];
            return cplex_l1_bellman::solve_bellman_state(in, *cplex_env);
          };
          auto t0 = std::chrono::steady_clock::now();
      ...
      if (run_gurobi) {
        try {
          ...
          auto gurobi_op = [&](int s, const std::vector<double>& v) {
            ...
            in.transitions = transitions_by_state[s];
            in.sigma = sigma_by_state[s];
            return gurobi_l1_bellman::solve_bellman_state(in, *gurobi_env);
          };
          auto t0 = std::chrono::steady_clock::now();
