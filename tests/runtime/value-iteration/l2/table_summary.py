...
def main():
    ...
    print(r"\centering")
    print(r"\caption{L2 robust value iteration (benchmarks).}")
    print(r"\begin{tabular}{l|ccc|c}")
    print(r"Instance & Fast (s) & CPLEX (s) & Mosek (s) & Iterations \\")
    print(r"\hline")
    ...
    for instance, data in rows:
        ...
        iteration_range = fmt_iteration_range([fast_iters, cplex_iters, mosek_iters])
        print(
            f"{label} & {fmt(fast_s)} & {fmt(cplex_s)} & {fmt(mosek_s)} & {iteration_range} \\\\"
        )
        if data.get("cplex_failed", "0") == "1":
            notes.append(f"CPLEX {label}")
        if data.get("gurobi_failed", "0") == "1":
            notes.append(f"Gurobi {label}")
        if data.get("fast_failed", "0") == "1":
            notes.append(f"Fast {label}")
