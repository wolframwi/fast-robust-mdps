...
ProjectionResult solve_projection_problem(const ProjectionInput& input, ::GRBEnv& env) {
  ...
  try {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      t.push_back(model.addVar(0.0, GRB_INFINITY, 0.0, GRB_CONTINUOUS));
    }
    ...
    for (int i = 0; i < n; ++i) {
      model.addConstr(t[i] >= p[i] - input.pbar[i]);
      model.addConstr(t[i] >= -p[i] + input.pbar[i]);
    }
    ...
    if (model.get(GRB_IntAttr_Status) != GRB_OPTIMAL) {
      throw std::runtime_error("Gurobi failed to solve L1 projection");
    }
    ...
    throw std::runtime_error("Gurobi L1 projection error: " + exc.getMessage());
  }
}
